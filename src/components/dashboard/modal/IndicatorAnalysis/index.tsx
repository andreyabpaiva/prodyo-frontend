...
const getActionStatus = (action: ApiAction): ModelsStatusEnum => {
    ...
    // Fall back to productivity level
    return productivityToStatus[action.cause.productivity_level] || ModelsStatusEnum.StatusNotStarted;
};

export function IndicatorAnalysis() {
    const router = useRouter();
    const queryClient = useQueryClient();
    const iterationId = useSelector((state: RootState) => state.iteration.activeGraphsId);
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await iterationService.causeActionList({ iterationId: iterationId ?? "" });
    ...
    const updateActionMutation = useMutation({
        mutationFn: async ({ actionId, assigneeId, status }: { actionId: string; assigneeId?: string; status?: string }) => {
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["cause-action-list", iterationId] });
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
                ...
                <div className="relative z-10 w-full max-w-2xl rounded-3xl border-[3px] border-black bg-white px-10 py-8">
                    ...
                    <p className="text-center text-black font-bold">Nenhuma causa ou ação encontrada</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-[680px] max-h-[90vh] overflow-y-auto rounded-3xl border-[3px] border-black bg-white px-8 py-6">
                ...
                <h2 className="text-2xl font-bold text-black mb-6">Análise do indicador</h2>
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="flex items-center gap-2">
                                                <p className="text-lg font-bold text-black mb-1">Causa</p>
                        ...
                        <div>
                            ...
                            {data.actions.length > 2 && (
                                <>
                                    ...
                                    <p className="text-[var(--disabled)]" >Arraste para o lado para visualizar mais</p>
                                </>
                            )}
                        </div>
                                        ...
                                        <div className="rounded-2xl border-[3px] border-black overflow-hidden shadow-[6px_6px_0px_rgba(0,0,0,0.35)] transition-transform hover:-translate-y-1">
                                            <div className="px-4 py-2 flex items-center justify-between gap-2 border-b-[3px] border-black bg-white">
                                                <div className="relative">
                                                    <button
                                                        type="button"
                                                        onClick={() => setOpenStatusId(openStatusId === action.id ? null : action.id)}
                                                    ...
                                                    >
                                                        <Badge className={`${statusTone[getActionStatus(action)]} border-3 text-xs font-bold rounded-full px-3 py-0.5`}>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    >
                                                        ...
                                                        <span>{action.assignee?.name || "Sem atribuição"}</span>
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    <p className="text-lg font-bold text-black mb-1">Ação</p>
                                                    <p className="text-xs font-bold uppercase text-black">
                                        ...
                                        {openStatusId === action.id && (
                                            <>
                                                ...
                                                <div className="absolute top-[44px] left-0 z-50 w-40 rounded-[12px] border-[3px] border-black bg-white shadow-lg overflow-hidden">
                                                    {(Object.entries(statusLabels) as [ModelsStatusEnum, string][]).map(([status, label], index) => (
                                                        <button
                                                            key={status}
                                                            type="button"
                                                            onClick={() => handleStatusSelect(action.id, status)}
                                                            disabled={updateActionMutation.isPending}
