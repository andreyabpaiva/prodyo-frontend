...
export default function CreateTaskForm({ projectId }: CreateTaskModalProps) {
  ...
  const activeIterationId = useAppSelector(
    (state) => state.iteration.activeIterationId,
  );
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[32px] border-[3px] border-dark bg-primary px-10 py-8">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="flex items-center gap-3">
            <Badge
              className={`${statusClasses[status]} rounded-full border-[3px] border-dark px-6 py-1 text-sm font-bold`}
            >
              {statusLabels[status]}
            </Badge>
            <UserSelect
