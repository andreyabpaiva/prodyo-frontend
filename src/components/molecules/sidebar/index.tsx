  ...
  return (
    <aside className="fixed top-14 left-0 h-[calc(100vh-3.5rem)] w-50 min-w-50 z-40 border-r-3 border-[--dark] bg-[--background] px-4 py-8 overflow-y-auto">
      <div className="flex h-full flex-col">
        ...
        >
          <span
            className="absolute left-[27px] top-4 bottom-2 w-[1px] bg-divider"
            aria-hidden
          />
          <div className="flex flex-col gap-2">
            {iterations.map((iteration) => {
              ...
              const isActive =
                activeState?.section === "iterations" &&
                activeState?.id === iteration.id;
              return (
                <Link
                ...
                return (
                  <Link
                    ...
                    onClick={() => {
                      if (iteration.id) {
                        router.push(
                          `/projects/${projectId}/indicators?iteration=${iteration.id}`
                        );
