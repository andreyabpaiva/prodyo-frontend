...
export function CreateImprovementForm({ taskId }: CreateImprovementModalProps) {
  ...
  const t = useTranslations("ImprovementForm");
  ...
  const nextImprovementNumber = useMemo(() => {
    ...
    const maxNumber = Math.max(
      ...improvements.map((imp: any) => imp.number || 0),
    );
    return maxNumber + 1;
  }, [improvements]);
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[32px] border-[3px] border-dark bg-primary px-10 py-8">
        ...
        <div className="mt-6 flex items-center">
          <h1 className="flex-1 text-3xl font-bold">
            {t("title", { number: nextImprovementNumber })}
          </h1>
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-alert border-[3px] border-dark text-sm font-bold text-dark">
            <input
              type="text"
              inputMode="numeric"
              value={points}
